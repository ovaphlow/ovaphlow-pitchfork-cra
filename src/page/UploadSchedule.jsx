...
export default function UploadSchedule() {
    ...
    const handle1 = () => new Promise((resolve, reject) => {
        fetch("/api/simple/biz/document", {
            method: "GET", // 默认 GET
        })
            .then((response) => {
        ...
        <Layout option={"上传作业计划"}>
            ...
            <div className="grid gap-4">
                <div className="bg-white p-4 shadow rounded">
                    ...
                    <p>
                        XMLHttpRequest
                        Ajax (jQuery) 是 异步网络请求，用来与后端程序通信
                        fetch, Axios 都是XMLHttpRequest的封装
                        fetch 为浏览器自带的工具，Axios需要单独安装或引入(import)
                        下一个版本的Node.js 会内置 fetch 方法
                    </p>
                    <p className="mt-4 text-2xl">{documentList.length}</p>
                    <button className="border bg-indigo-500 shadow text-white p-4" onClick={click}>111</button>
                </div>
            </div>
        </Layout>
    )
